...
int Adafruit_AMG88xx::_i2c_init(uint8_t _i2caddr) {
	/*Wire.begin();*/
	return wiringPiI2CSetup(_i2caddr);
}
...
void Adafruit_AMG88xx::read(uint8_t reg, uint8_t *buf, uint8_t num) {
	...
	while (pos < num) {
		...
		wiringPiI2CWriteReg8(I2C_FD, reg, (uint8_t)reg + pos);
		//wiringPiI2CWrite(I2C_FD, (uint8_t)reg + pos);
		...
		for (int i = 0; i < read_now; i++) {
			//buf[pos] = Wire.read();
			//buf[pos] = wiringPiI2CRead(I2C_FD);
			buf[pos] = wiringPiI2CReadReg8(I2C_FD, (uint8_t)reg + pos);
...
void Adafruit_AMG88xx::write(uint8_t reg, uint8_t *buf, uint8_t num) {
	...
	wiringPiI2CWriteReg8(I2C_FD, reg, reg);
	for (int i = 0; i < num; i++) {
		//wiringPiI2CWrite(I2C_FD, buf[i]);
		wiringPiI2CWriteReg8(I2C_FD, reg + i, buf[i]);
	}
}
